...
class GameModelTests( TestCase ):
    ...
    def test_is_game_over_is_false_if_guesses_left( self ):
        ...
        game.handleGuess('X')
        #this will verify that game.is_game_over is false since we
        #still have guesses left (letter we guessed is not in word)
    ...
    def test_is_game_over_is_false_if_not_all_letters_guessed( self ):
        ...
        game.handleGuess('T')
        #this will verify that game.is_game_over is false since we
        #still have guesses left (letter we guessed is in word)
        self.assertFalse(game.is_game_over)
    ...
    def test_is_game_over_is_true_if_no_guesses_left( self ):
        game = Game( 
            word= 'TESTWORD',
            guessed_word_state= ['','','S','','W','O','R',''],
            letters_guessed = ['S', 'A', 'W', 'O', 'R','C','B','K','Z'],
            guesses_allowed= 10, 
            guesses_taken= 9
        )
        game.handleGuess('X')
        #this will verify that game.is_game_over is true since we
